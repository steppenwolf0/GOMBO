...
globalBest=float(sys.argv[15])

log="	%d	%d	%d	%d	%d	%d	%d	%d	%d	%d	%d	%d	%d	%d	%g \n"%(iterMax,w1,w2,w3,w4,h1,h2,h3,wd1,wd2,wd3,index,kfoldIndex,generation,globalBest)
print(log)
#Model declaration************************************************************************
...
while ((valid_accuracy<limit) & (iter<iterMax)):
	...
	if np.var(repeatWindow)==0 and iter<5:
		iter=iterMax
