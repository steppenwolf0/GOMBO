...
void parseFOSElementSize( int *index, int argc, const char** argv );
...
void parseParameters( int argc, const char **argv, int *index )
{
    ...
    noError = 1;
    //noError = noError && sscanf( argv[*index+0], "%d", &number_of_parameters );
...
void initializeMemory( void )
{
	...
	initializeIndividuals( maximum_number_of_populations);
...
void initializeNewPopulationMemory( int population_index )
{
	...
	initializePopulations( population_index,  number_of_parameters, population_sizes,  selection_sizes);
...
void initializeFOS( int population_index )
{
    ...
    linkage_model[population_index] = copyFOS(new_FOS);
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	//linkage_model[population_index] = new_FOS;
	ezilaitiniFOS(new_FOS);
	...
	// parallelize this chunk of code
		...
		for (j = 0; j<population_sizes[population_index]; j++)
		{
			...
			for (k = 0; k < number_of_parameters; k++)
				populations[population_index][j]->parameters[k] = lower_init_ranges[k] + (upper_init_ranges[k] - lower_init_ranges[k])*randomRealUniform01();

			//printf("%d\n",j);
			//printf("label2\n");
			//printf("%f\n", populations[population_index][j].parameters[number_of_parameters - 1]);
		...
		for (j = 0; j < population_sizes[population_index]; j++)
		{
			installedProblemEvaluation(populations[population_index][j], number_of_parameters, NULL, NULL, 0, 0);
		}
	//printf("label1\n");
...
void computeRanksForOnePopulation( int population_index )
{
    ...
    {
		double* objective_valuesTemp= (double *)Malloc(population_sizes[population_index] *sizeof(double ));
		double* constraint_valuesTemp = (double *)Malloc(population_sizes[population_index] *sizeof(double));
		int j;
		for (j = 0; j < population_sizes[population_index]; j++)
		{
			objective_valuesTemp[j] = populations[population_index][j]->objective_value;
			constraint_valuesTemp[j] = populations[population_index][j]->constrained_value;
		...
		sorted = mergeSortFitness(objective_valuesTemp, constraint_valuesTemp, population_sizes[population_index]);

        rank                               = 0;
        ranks[population_index][sorted[0]] = rank;
        for( i = 1; i < population_sizes[population_index]; i++ )
        {
            if( populations[population_index][sorted[i]]->objective_value != populations[population_index][sorted[i-1]]->objective_value)
		...
		free(objective_valuesTemp);
		free(constraint_valuesTemp);
        free( sorted );
    }
}
...
void writeGenerationalStatisticsForOnePopulation( int population_index )
{
    ...
    population_objective_best   = populations[population_index][0]->objective_value;
    population_constraint_best  = populations[population_index][0]->constrained_value;
    population_objective_worst  = populations[population_index][0]->objective_value;
    population_constraint_worst = populations[population_index][0]->constrained_value;
    for( j = 0; j < population_sizes[population_index]; j++ )
    {
        population_objective_avg  += populations[population_index][j]->objective_value;
        population_constraint_avg += populations[population_index][j]->constrained_value;
        if( betterFitness( population_objective_worst, population_constraint_worst, populations[population_index][j]->objective_value,
			populations[population_index][j]->constrained_value ) )
        {
            population_objective_worst = populations[population_index][j]->objective_value;
            population_constraint_worst = populations[population_index][j]->constrained_value;
        }
        if( betterFitness(populations[population_index][j]->objective_value,
			populations[population_index][j]->constrained_value, population_objective_best, population_constraint_best ) )
        {
            population_objective_best = populations[population_index][j]->objective_value;
            population_constraint_best = populations[population_index][j]->constrained_value;
        }
    }
    ...
    for( j = 0; j < population_sizes[population_index]; j++ )
    {
        population_objective_var  += (populations[population_index][j]->objective_value - population_objective_avg)*(populations[population_index][j]->objective_value - population_objective_avg);
        population_constraint_var += (populations[population_index][j]->constrained_value - population_constraint_avg)*(populations[population_index][j]->constrained_value - population_constraint_avg);
    }
...
void writeGenerationalSolutions( short final )
{
    ...
    for( i = 0; i < number_of_populations; i++ )
    {
        ...
        for( j = 0; j < population_sizes[i]; j++ )
        {
            for( k = 0; k < number_of_parameters; k++ )
            {
                sprintf( string, "%13e", populations[i][j]->parameters[k] );
            ...
            fputs( string, file_all );
            fputs( string, file_population );
            sprintf( string, "%13e %13e", populations[i][j]->objective_value, populations[i][j]->constrained_value);
            fputs( string, file_all );
            fputs( string, file_population );
            sprintf( string, "\n" );
            fputs( string, file_all );
        ...
        if( number_of_generations[i] > 0 && !final )
        {
            for( j = 0; j < selection_sizes[i]; j++ )
            {
                for( k = 0; k < number_of_parameters; k++ )
                {
                    sprintf( string, "%13e", selections[i][j]->parameters[k] );
                ...
                sprintf( string, "%13e %13e", selections[i][j]->objective_value, selections[i][j]->constrained_value );
                fputs( string, file_selection );
                sprintf( string, "\n" );
                fputs( string, file_selection );
            }
...
void writeGenerationalSolutionsBest( short final )
{
    ...
    for( i = 0; i < number_of_parameters; i++ )
    {
        sprintf( string, "%13e\n", populations[population_index_best][individual_index_best]->parameters[i] );
        fputs( string, file );
        if( i < number_of_parameters-1 )
        {
            //sprintf( string, " " );
    ...
    sprintf( string, "     " );
    fputs( string, file );
    sprintf( string, "%13e %13e", populations[population_index_best][individual_index_best]->objective_value,
		populations[population_index_best][individual_index_best]->constrained_value);
	printf("%13e %13e \n", populations[population_index_best][individual_index_best]->objective_value,
		populations[population_index_best][individual_index_best]->constrained_value);
    fputs( string, file );
    sprintf( string, "\n" );
    fputs( string, file );
...
short checkNumberOfEvaluationsTerminationCondition( void )
{
	if (number_of_evaluations >= maxEval)
		return(1);
...
void checkAverageFitnessTerminationCondition( void )
{
    ...
    for( i = number_of_populations-1; i >= 0; i-- )
    {
        ...
        for( j = 0; j < population_sizes[i]; j++ )
        {
            average_objective_values[i] += populations[i][j]->objective_value;
            average_constraint_values[i] += populations[i][j]->constrained_value;
        }
...
void determineBestSolutionInCurrentPopulations( int *population_of_best, int *index_of_best )
{
    ...
    for( i = 0; i < number_of_populations; i++ )
    {
        for( j = 0; j < population_sizes[i]; j++ )
        {
            if( betterFitness( populations[i][j]->objective_value, populations[i][j]->constrained_value,
                               populations[(*population_of_best)][(*index_of_best)]->objective_value,
				populations[(*population_of_best)][(*index_of_best)]->constrained_value) )
            {
...
short checkFitnessVarianceTerminationSinglePopulation( int population_index )
{
    ...
    for( i = 0; i < population_sizes[population_index]; i++ )
        objective_avg  += populations[population_index][i]->objective_value;
    objective_avg = objective_avg / ((double) population_sizes[population_index]);

    objective_var = 0.0;
    for( i = 0; i < population_sizes[population_index]; i++ )
        objective_var  += (populations[population_index][i]->objective_value -objective_avg)*(populations[population_index][i]->objective_value -objective_avg);
    ...
    else
    {
        for( i = 0; i < selection_sizes[population_index]; i++ )
        {
            ...
            /*for( j = 0; j < number_of_parameters; j++ )
                selections[population_index][i].parameters[j] = populations[population_index][sorted[i]].parameters[j];

			selections[population_index][i].objective_value  = populations[population_index][sorted[i]].objective_value;
            selections[population_index][i].constrained_value = populations[population_index][sorted[i]].constrained_value;*/
			
			copyIndividual(selections[population_index][i], populations[population_index][sorted[i]], number_of_parameters);
			//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
        }
    }
...
void makeSelectionsForOnePopulationUsingDiversityOnRank0( int population_index )
{
    ...
    index_of_farthest    = 0;
    distance_of_farthest = populations[population_index][preselection_indices[0]]->objective_value;
    for( i = 1; i < number_of_rank0_solutions; i++ )
    {
        if( populations[population_index][preselection_indices[i]]->objective_value > distance_of_farthest )
        {
            index_of_farthest    = i;
            distance_of_farthest = populations[population_index][preselection_indices[i]]->objective_value;
        }
    }
    ...
    for( i = 0; i < number_of_rank0_solutions; i++ )
        nn_distances[i] = distanceEuclidean( populations[population_index][preselection_indices[i]]->parameters, populations[population_index][selection_indices[number_selected_so_far-1]]->parameters, number_of_parameters );
    ...
    while( number_selected_so_far < selection_sizes[population_index] )
    {
        ...
        for( i = 0; i < number_of_rank0_solutions; i++ )
        {
            value = distanceEuclidean( populations[population_index][preselection_indices[i]]->parameters, populations[population_index][selection_indices[number_selected_so_far-1]]->parameters, number_of_parameters );
    ...
    for( i = 0; i < selection_sizes[population_index]; i++ )
    {
		//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
        /*for( j = 0; j < number_of_parameters; j++ )
            selections[population_index][i].parameters[j] = populations[population_index][selection_indices[i]].parameters[j];

        selections[population_index][i].objective_value  = populations[population_index][selection_indices[i]].objective_value;
        selections[population_index][i].constrained_value = populations[population_index][selection_indices[i]].constrained_value;*/

		copyIndividual(selections[population_index][i], populations[population_index][selection_indices[i]],  number_of_parameters);
		//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    }
    ...
    for( i = 0; i < linkage_model[population_index]->length; i++ )
        if( distribution_multipliers[population_index][i] < 1.0 )
            for( j = 0; j < linkage_model[population_index]->set_length[i]; j++)
                mean_vectors[population_index][linkage_model[population_index]->sets[i][j]] = selections[population_index][0]->parameters[linkage_model[population_index]->sets[i][j]];
...
void estimateMeanVectorML( int population_index )
{
    ...
    for( i = 0; i < number_of_parameters; i++ )
    {
        ...
        for( j = 0; j < selection_sizes[population_index]; j++ )
            new_mean += selections[population_index][j]->parameters[i];
    ...
    for( i = 0; i < number_of_parameters; i++ )
    {
        for( j = 0; j < number_of_parameters; j++ )
        {
            ...
            for( m = 0; m < selection_sizes[population_index]; m++ )
                cov += (selections[population_index][m]->parameters[i]-mean_vectors[population_index][i])*(selections[population_index][m]->parameters[j]-mean_vectors[population_index][j]);
        ...
        for( j = 0; j < linkage_model[population_index]->set_length[i]; j++ )
        {
            ...
            for( k = j; k < linkage_model[population_index]->set_length[i]; k++ )
            {
                ...
                else
                {
                    cov = 0.0;
                    for( m = 0; m < selection_sizes[population_index]; m++ )
                        cov += (selections[population_index][m]->parameters[vara]-mean_vectors[population_index][vara])*(selections[population_index][m]->parameters[varb]-mean_vectors[population_index][varb]);
...
void copyBestSolutionsToPopulation( int population_index )
{
    ...
    if( !populations_terminated[population_index] )
    {
		//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
       /* for( k = 0; k < number_of_parameters; k++ )
            populations[population_index][0].parameters[k] = selections[population_index][0].parameters[k];

		populations[population_index][0].objective_value = selections[population_index][0].objective_value;
		populations[population_index][0].constrained_value = selections[population_index][0].constrained_value;*/
		copyIndividual(populations[population_index][0], selections[population_index][0], number_of_parameters);
		//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    }
}
...
void getBestInPopulation( int population_index, int *individual_index )
{
    ...
    *individual_index = 0;
    for( i = 0; i < population_sizes[population_index]; i++ )
        if( betterFitness(populations[population_index][i]->objective_value, populations[population_index][i]->constrained_value,
			populations[population_index][*individual_index]->objective_value, populations[population_index][*individual_index]->constrained_value))
            *individual_index = i;
}
...
void getOverallBest( int *population_index, int *individual_index )
{
    ...
    *individual_index = best_individual_index;
    for( i = 0; i < number_of_populations; i++ )
    {
        ...
        if( betterFitness(populations[i][best_individual_index]->objective_value, populations[i][best_individual_index]->constrained_value,
			populations[*population_index][*individual_index]->objective_value, populations[*population_index][*individual_index]->constrained_value))
        {
            *population_index = i;
            *individual_index = best_individual_index;
...
short generateNewSolutionFromFOSElement( int population_index, int FOS_index, int individual_index, short apply_AMS )
{
    ...
    double *result, *individual_backup, delta_AMS, shrink_factor;
	...
	Individual* temp = initializeIndividual(number_of_parameters);
	copyIndividual(temp, populations[population_index][individual_index],number_of_parameters);
    ...
    if( apply_AMS && (number_of_generations[population_index] > 0) )
    {
        ...
        while( (out_of_range == 1) && (shrink_factor > 1e-10) )
        {
            ...
            {
                im = indices[m];
                result[m] = populations[population_index][individual_index]->parameters[im]+shrink_factor*delta_AMS*distribution_multipliers[population_index][FOS_index]*mean_shift_vector[population_index][im];
        ...
        if( !out_of_range )
        {
            for( m = 0; m < num_indices; m++ )
            {
                populations[population_index][individual_index]->parameters[indices[m]] = result[m];
            }
        }
    }

	double objective_value_before = populations[population_index][individual_index]->objective_value;
	double constraint_value_before = populations[population_index][individual_index]->constrained_value;

    installedProblemEvaluation( populations[population_index][individual_index], num_touched_indices, touched_indices,
		individual_backup, objective_value_before, constraint_value_before);
    improvement = betterFitness(populations[population_index][individual_index]->objective_value,
		populations[population_index][individual_index]->constrained_value, objective_value_before, constraint_value_before);
    if( improvement )
    {
        any_improvement = 1;
  //      populations[population_index][individual_index].objective_value = obj_val;
		//populations[population_index][individual_index].constrained_value = cons_val;
        /*for( j = 0; j < num_touched_indices; j++ )
            individual_backup[j] = populations[population_index][individual_index].parameters[touched_indices[j]];*/
		copyIndividual(temp, populations[population_index][individual_index], number_of_parameters);
    }
    ...
    if( !any_improvement && randomRealUniform01() >= 0.05 )
    {
		...
		populations[population_index][individual_index].objective_value = objective_value_before;
		populations[population_index][individual_index].constrained_value = constraint_value_before;*/

		copyIndividual(populations[population_index][individual_index], temp, number_of_parameters);
    }
   /* else
    {
...
short applyAMS( int population_index, int individual_index )
{
    ...
    double shrink_factor, delta_AMS, *solution_AMS;
    int m;
    ...
    while( (out_of_range == 1) && (shrink_factor > 1e-10) )
    {
        ...
        {
            solution_AMS[m] = populations[population_index][individual_index]->parameters[m]
	...
	Individual* ind = initializeIndividual(number_of_parameters);
	for (m = 0; m < number_of_parameters; m++)
		ind->parameters[m] = solution_AMS[m];
	
	
    if( !out_of_range )
    {
        installedProblemEvaluation( ind,number_of_parameters, NULL, NULL, 0, 0 );
        if( randomRealUniform01() < 0.05 || betterFitness(ind->objective_value, ind->constrained_value,
			populations[population_index][individual_index]->objective_value, populations[population_index][individual_index]->constrained_value))
        {

			copyIndividual(populations[population_index][individual_index], ind, number_of_parameters);
...
void applyForcedImprovements( int population_index, int individual_index, int donor_index )
{
    ...
    while( alpha >= 0.01 )
    {
        ...
        order = randomPermutation( linkage_model[population_index]->length );
			...
			for (io = 0; io < linkage_model[population_index]->length; io++)
			{
				...
				double* FI_backup = (double*)Malloc(num_touched_indices*sizeof(double));
				for (j = 0; j < num_touched_indices; j++)
				{
					FI_backup[j] = populations[population_index][individual_index]->parameters[touched_indices[j]];
					populations[population_index][individual_index]->parameters[touched_indices[j]] = alpha*populations[population_index][individual_index]->parameters[touched_indices[j]] + (1 - alpha)*populations[population_index][donor_index]->parameters[touched_indices[j]];
				}
				double objective_value_before = populations[population_index][individual_index]->objective_value;
				double constraint_value_before = populations[population_index][individual_index]->constrained_value;

				installedProblemEvaluation(populations[population_index][individual_index], num_touched_indices, touched_indices, FI_backup,
				...
				{
					/*for (j = 0; j < num_touched_indices; j++)
						populations[population_index][individual_index].parameters[touched_indices[j]] = FI_backup[j];
					populations[population_index][individual_index].objective_value = objective_value_before;
					populations[population_index][individual_index].constrained_value = constraint_value_before;*/
	...
	if (!improvement)
    {
		...
		//printf("label0");
		copyIndividual(populations[population_index][individual_index], populations[population_index][donor_index], number_of_parameters);
    }
}
...
short generationalImprovementForOnePopulationForFOSElement( int population_index, int FOS_index, double *st_dev_ratio )
{
    ...
    {
        if( betterFitness( populations[population_index][i]->objective_value, populations[population_index][i]->constrained_value,
			populations[population_index][index_best_population]->objective_value, populations[population_index][index_best_population]->constrained_value ) )
            index_best_population = i;

        if( betterFitness( populations[population_index][i]->objective_value, populations[population_index][i]->constrained_value,
                           selections[population_index][0]->objective_value, selections[population_index][0]->constrained_value ) )
        {
            number_of_improvements++;
            for( j = 0; j < num_indices; j++ )
                average_parameters_of_improvements[j] += populations[population_index][i]->parameters[indices[j]];
        }
    }
...
void ezilaitiniMemory( void )
{
    ...
    for( i = 0; i < number_of_populations; i++ )
    {
		for (j = 0; j < population_sizes[i]; j++)
		{
			freeIndividual(populations[i][j]);
...
Individual* run( void )
{
	...
	int bestIndex;
	bestIndexPointer =&bestIndex;
	
	int *bestPopulationPointer;
	int bestPopulation;
	bestPopulationPointer =&bestPopulation;
	
	getOverallBest(bestPopulationPointer,bestIndexPointer);

	Individual* output = initializeIndividual(number_of_parameters);
	copyIndividual(output, populations[bestPopulation][bestIndex],number_of_parameters);

	/*int i;
	for (i = 0; i < number_of_parameters; i++)
	{
		printf("%.4f",output->parameters[i]);
	}*/
    ...
    printf("bestPopulation %d\n", bestPopulation);
	printf("bestIndex %d\n", bestIndex);

	/*-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*/
	//from individidual declaration
	/*-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*/
